...
parser.add_argument("--num_outer_loop_iters", default=1, type=int) # default = 25  #SGD
...
class Proxy:
    ...
    def train(self, dataset):
        ...
        for i in range(self.args.proxy_num_iterations+1):
            ...
            #print(f"Mytype:{type(loss.item())}")
            loss.backward()
            last_losses.append((loss.item(),))
            train_losses.append((loss.item(),))
            #print(f"Mytype:{train_losses}")
            opt.step()
            opt.zero_grad()
...
def main(args):
    bpath = "mols/data/blocks_PDB_105.json"
    device = torch.device('cuda')
    ...
    proxy_dataset.load_h5("mols/data/docked_mols.h5", args, num_examples=args.num_init_examples)
