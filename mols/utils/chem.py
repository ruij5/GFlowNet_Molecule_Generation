...
def onehot(arr, num_classes, dtype=np.intc):
    arr = np.asarray(arr, dtype=np.intc)
...
class DockVina_smi:
    def __init__(self,
                 outpath,
                 mgltools_dir=os.path.join("mols/Programs", "mgltools_x86_64Linux2_1.5.6"),
                 vina_dir=os.path.join("mols/Programs", "vina"),
                 docksetup_dir=os.path.join("mols/data", "mapk1"),
                 rec_file="mapk1.pdbqt",
                 bindsite=(-14.64, -36.62, -24.73, 15, 15, 15),
                 dock_pars="",
                 cleanup=True):
        import os
